...
import { CommentTargetType } from '@app/modules/post-comments/types/comments.interface';
...
export class PostCountUpdatingListener {
  ...
  async incrementCommentCounter(event: CommentCreateEvent) {
    if (event.targetType === CommentTargetType.POST) {
      await this.postService.updateCounters(event.targetId, {
        commentsCount: 1,
      });
    }
  }
  ...
  async decrementCommentCounter(event: CommentHardDeleteEvent) {
    if (event.targetType === CommentTargetType.POST) {
      await this.postService.updateCounters(event.parentId, {
        commentsCount: -1,
      });
    }
  }
  ...
  async incrementLikesCount(event: PostLikeEvent) {
    await this.postService.updateCounters(event.postId, { likesCount: 1 });
  }
